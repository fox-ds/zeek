...
bool CompositeHash::RecoverOneVal(const HashKey& hk, Type* t, ValPtr* pval, bool optional, bool singleton) const
	...
	case TYPE_INTERNAL_ADDR:
		...
		IPAddr addr(IPv6, reinterpret_cast<const uint32_t*>(hk.KeyAtRead()), IPAddr::Network);
	...
	case TYPE_INTERNAL_OTHER:
		...
		case TYPE_FUNC:
			...
			if ( ! f )
				reporter->InternalError("failed to look up unique function id %" PRIu32
				                        " in CompositeHash::RecoverOneVal()", id);
		...
		case TYPE_PATTERN:
			...
			texts[0] = reinterpret_cast<const char*>(hk.KeyAtRead());
			hk.SkipRead("pattern-string1", strlen(texts[0]) + 1);
			texts[1] = reinterpret_cast<const char*>(hk.KeyAtRead());
...
bool CompositeHash::SingleValHash(HashKey &hk, const Val* v, Type* bt, bool type_check, bool optional, bool singleton) const
	{
	...
	case TYPE_INTERNAL_ADDR:
		...
		v->AsAddr().CopyIPv6(reinterpret_cast<uint32_t*>(hk.KeyAtWrite()));
	...
	case TYPE_INTERNAL_SUBNET:
		...
		v->AsSubNet().Prefix().CopyIPv6(reinterpret_cast<uint32_t*>(hk.KeyAtWrite()));
	...
	case TYPE_INTERNAL_OTHER:
		...
		case TYPE_PATTERN:
			...
			hk.Write("pattern-string1", reinterpret_cast<const void*>(texts[0]), lens[0]);
			hk.Write("pattern-string2", reinterpret_cast<const void*>(texts[1]), lens[1]);
			break;
			}
...
bool CompositeHash::ReserveSingleTypeKeySize(HashKey& hk, Type* bt, const Val* v, bool type_check,
					     bool optional, bool calc_static_size, bool singleton) const
	{
	InternalTypeTag t = bt->InternalType();
	...
	case TYPE_INTERNAL_OTHER:
		...
		case TYPE_RECORD:
			...
			for ( int i = 0; i < num_fields; ++i )
				...
				if ( ! ReserveSingleTypeKeySize(hk, rt->GetFieldType(i).get(),
				                                rv_v.get(), type_check, optional_attr,
		...
		case TYPE_TABLE:
			...
			for ( auto& kv : *hashkeys )
				...
				if ( ! bt->IsSet() )
					{
					auto val = const_cast<TableVal*>(tv)->FindOrDefault(key);
					if ( ! ReserveSingleTypeKeySize(hk, val->GetType().get(), val.get(), type_check,
		...
		case TYPE_VECTOR:
			...
			for ( unsigned int i = 0; i < vv->Size(); ++i )
				...
				if ( val && ! ReserveSingleTypeKeySize(hk, bt->AsVectorType()->Yield().get(),
		...
		case TYPE_LIST:
			...
			for ( int i = 0; i < lv->Length(); ++i )
				{
				if ( ! ReserveSingleTypeKeySize(hk, lv->Idx(i)->GetType().get(), lv->Idx(i).get(),
