 ...
 * e.g. for lookups in hash-tables and connection ID generation.
 *
...
public:
	...
	template<typename T>
	void ReserveType(const char* tag) { Reserve(tag, sizeof(T), sizeof(T)); }
	void Reserve(const char* tag, size_t addl_size, size_t alignment = 0);
 
	// Allocates the reserved amount of memory
	void Allocate();

	// Incremental writes into an allocated HashKey.
...
protected:
	...
	// Payload setters for types stored directoly in the key_u union.
	void Set(bool b);
	void Set(int i);
